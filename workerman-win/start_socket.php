...
$ws_worker = new Worker("websocket://0.0.0.0:{$tp_config['socket2']}"); // 16668
//启动4个进程对外提供服务  
$ws_worker->count = 1;
    ...
    if ($dataInfo && isset($dataInfo['operation'])) {
        switch ($dataInfo['operation']) {
            case "CreateRoom"://创建房间
                $url = $baseUrl . "game.php/Game/CreateRoom";
            ...
            case "PrepareJoinRoom"://准备进入房间
                //请求控制器
                $url = $baseUrl . "game.php/Game/PrepareJoinRoom";
            ...
            case "ReadyStart"://游戏开始
                //请求控制器
                $url = $baseUrl . "game.php/Game/ReadyStart";
                $returnData = https_post($data, $url);
				var_dump($returnData);
...
//                            }
            ...
            case "GrabBanker"://抢庄
                //请求控制器
                $url = $baseUrl . "game.php/Game/GrabBanker";
                $returnData = https_post($data, $url);
                //向客户端推送数据
                $resultData = json_decode($returnData, true);
                if (is_array($resultData)) {
            ...
            case 'PlayerMultiples'://手动下注
                $url = $baseUrl.'game.php/Game/PlayerMultiples';
...
//请求控制器
                $url = $baseUrl . "game.php/Game/PlayerShowCard";
                $returnData = https_post($data, $url);
...
$ws_worker->onClose = function($connection) use($ws_worker, &$connection_all, &$timer_all) {
    ...
    $onlineUrl = $baseUrl."game.php/Game/onlineStatus";
...
function BroadcastVoice($data) {
    ...
    $url = $baseUrl.'game.php/Game/BroadcastVoice';
    $result = https_post($data, $url);
...
function GameResult($room_id) {//发送房间的游戏结果
    ...
    $url = $baseUrl.'game.php/Game/GameResult';
...
function AutoStartShow($room_id, $timer_all) {//自动摊牌回调函数
    ...
    $url = $baseUrl.'game.php/Game/AutoStartShow';
    $result = https_post(json_encode(['room_id' => $room_id]), $url);
...
function AutoStartMultiples($room_id, $timer_all) {//自动下注回调函数
    ...
    $url = $baseUrl.'game.php/Game/AutoStartMultiples';
    $result = https_post(json_encode(['room_id' => $room_id]), $url);
...
function GameStartTime($room_id, $timer_all) {
    ...
    $url = $baseUrl.'game.php/Game/GameStartTime';
    $result = https_post(json_encode(['room_id' => $room_id]), $url);
...
function GameAutoStart($room_id, $timer_all) {
    ...
    $url = $baseUrl.'game.php/Game/GameAutoStart';
    $result = https_post(json_encode(['room_id' => $room_id]), $url);
