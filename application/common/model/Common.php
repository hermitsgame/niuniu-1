	...
	public function _list($map = array(), $view = NULL, $order = '', $pagesize = 7, $table = '', $items = 'list', $pages = 'pages'){
		...
		//把得到的数据填充到模板中，以便显示在页面上
                foreach ($list as $key => $value) {
					$money = Db::query('select sum(scorpe) as money from t_history where member_id=:id group by member_id limit 1', ['id' => $list[$key]['id']]);
					isset($money[0]['money']) ? $list[$key]['money'] = $money[0]['money'] : $list[$key]['money'] = 0;
