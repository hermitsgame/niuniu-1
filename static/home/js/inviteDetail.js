...
app.controller("myCtrl", function ($scope, $http) {
    ...
    globalData.inviteName = Base64.decode(globalData.inviteName);
    ...
    $scope.joinImg = globalData.imageUrl + 'static/home/image/union_join.png';
    $scope.qrImg = globalData.qrUrl;

    document.title = globalData.inviteName + '的邀请函';
    ...
    $scope.httpModule = {
        joinGroup: function () {
            $http({
                url: globalData.baseUrl + 'game.php/gscore/applyJoinGroup',
                method: 'POST',
                header: { 'Content-Type': 'application/x-www-form-urlencoded' },
                data: {
                    "dealer_num": globalData.dealerNum,
                    "code": globalData.inviteCode,
                    "account_id": userData.accountId,
                }
            }).success(function (data, header, config, status) {
                    
                    if (data.result == 0) {
                        $scope.resultText = data.result_message;
                        setTimeout(function () {
